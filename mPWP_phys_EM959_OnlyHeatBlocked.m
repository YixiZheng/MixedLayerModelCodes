function mPWP_phys(met_input_file, profile_input_file, pwp_output_file,Agrow,Amelt,Aicecover)
...
% ad=min(dep_max.profile.z(abs(t_ad-nanmax(t_ad))<abs(0.1*nanmax(t_ad))))
...
for m = 1:nmet
    ...
    %%%%%%%%%%%%%%%%%% the main line
	Aactive=pwpgo(qi(m),shum(m),tair(m),lw(m),qi(m),UU(m),tx(m),ty(m),precip(m),m,Agrow,Amelt,Aicecover);
    %%%%%%%%%%%%%%%%%% the main line
...
function Aactive=pwpgo(qi,shum,tair,lw,sw,UU,tx,ty,precip,m,Agrow,Amelt,Aicecover) % Yixi 23/01/2019
...
%Advection term, using U as 0.01m/s, and a dt/dx term of 0.001s/m

% ad_i = floor(ad/dz);
% 
% if iter > 1
%     dtemp = ((-0.01*(t(ad_i:end)-t_orig(ad_i:end)))*0.01);
%     t(ad_i:end) = t(ad_i:end) + dtemp;
%     ds = ((-0.01*(s(ad_i:end)-s_orig(ad_i:end)))*0.01);
...
if t(1) < t_fp %starts forming sea ice, sets temp to freezing point
    ...
    h_i = h_i - (mr*ml_depth);
    Aicethermodynamics = Agrow;
elseif h_i > 0
    mr =[(d(1)/1000)*(cpw/Lf)*(t(1)- t_fp)];
    t(1:ml_index) = t_fp;
    s(1:ml_index) = s(1:ml_index)/(1+(Amelt*mr));
    h_i = h_i - (mr*ml_depth);
    Aicethermodynamics = Amelt;
else
    mr = 0;
%%%%%%%%%%%%%%%%%%%%% 27/Feb/2022
%%%%%%%%%%%%%%%%%%%%% Yixi changed this one, so that A can be tuned
    Aicethermodynamics = 0;
%%%%%%%%%%%%%%%%%%%%% 27/Feb/2022
%%%%%%%%%%%%%%%%%%%%% Yixi changed this one, so that A can be tuned
    h_i = 0;
end
...
% fw_flux = (s(1)*(((emp))-A*(mr*dz/dt))); 
fw_flux = (s(1)*(((1-0)*(emp))-Aicethermodynamics*(mr*dz/dt))); %20/Dec, Yixi changed A to Aicethermodynamics and Aicecover, as it captures a more reliable ice forming area
